...
public class ProductController {
    ...
    @GetMapping("/bulk")
    public ResponseEntity<PageResponse<ProductResponse>> getAllProducts( @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable ) {
    ...
    public ResponseEntity<PageResponse<ProductResponse>> getProductsByStore( @PathVariable UUID storeId, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable ) {
        PageResponse<ProductResponse> productResponse = productService.getProductsByStoreId(storeId, pageable);
        return ResponseEntity.ok(productResponse);
    }

    @GetMapping("/category/{categoryId}")
    public ResponseEntity<PageResponse<ProductResponse>> getProductsByCategory( @PathVariable UUID categoryId, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable ) {
        PageResponse<ProductResponse> productResponse = productService.getProductsByCategoryId(categoryId, pageable);
        return ResponseEntity.ok(productResponse);
    }

    @GetMapping("/brand/{brandId}")
    public ResponseEntity<PageResponse<ProductResponse>> getProductsByBrand( @PathVariable UUID brandId, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable ) {
        PageResponse<ProductResponse> productResponse = productService.getProductsByBrandId(brandId, pageable);
        return ResponseEntity.ok(productResponse);
    }

    @GetMapping("/active")
    public ResponseEntity<PageResponse<ProductResponse>> getActiveProducts( @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable ) {
